...
const render = (data) => {
    const html = data.map(elem => {
        ...
        <strong style="color:white; font-size: 25px;">${elem.email}</strong> 
        <p style="color:red; font-size: 25px;">[${elem.time}]</p>
        <i style="color:chocolate; font-size: 25px;">${elem.text}</i></div>`)}).join(" ");
...
sumbitButton.addEventListener("click", () => addProducto());
...
socket.on("MensajeIndividual", data => {
    ...
    <div style="display:flex; column-gap: 0.2rem;">
        <strong style="color:white; font-size: 25px;">${data.email}</strong> 
        <p style="color:red; font-size: 25px;">[${data.time}]</p>
        <i style="color:green; font-size: 25px;">${data.text}</i></div>               
`
    window.scrollTo(0, document.body.scrollHeight);
});
